...
const DevelopmentAndApprovalProjectDocumentationTable = ({callback}) => {
    const [rows, setRows] = useState([{id: 0, event: "", deadline: "", result: ""}])


    const addRow = () => {
        const emptyRow = {
            id: rows.length > 0 ? rows.at(-1).id + 1 : 0,
            event: "",
            deadline: "",
            result: ""
        }
    ...
    return (
        <TableContainer component={Paper}>
            <Table>
                <TableHead>
                    <TableRow>
                        <TableCell colSpan={4} align="center"><h3>Этап "Разработка и
                            утверждение проектной документации"</h3></TableCell>
                    </TableRow>
                    <TableRow>
                        ...
                        rows.map((row, index) =>
                            <TableRow key={index}>
                                ...
                                <TableCell style={{cursor: "pointer"}}
                                           onClick={() => deleteRow(row.id)}>x</TableCell>
                            </TableRow>
                        )
                    }
...
export default DevelopmentAndApprovalProjectDocumentationTable